...
> 2023-06 에 velog 에 정리한 글을 5-6년차 백엔드 시각으로 다시 손본 글입니다. 테스트 학습 시리즈의 첫 글이며, 이후 [의존성과 Testability](/posts/dependency-and-testability) → [레이어별 테스트 전략](/posts/layered-testing-strategy) → [외부 연동을 다루는 방법](/posts/handling-external-integrations) 으로 이어집니다.

**TL;DR.** Test Double 다섯 가지 (Dummy / Fake / Stub / Mock / Spy) 의 차이를 한 표로 끝냅니다. 도메인 협력자는 **Fake**, 외부 시스템 응답은 **Stub/Mock** — 이 두 줄이 글 전체의 핵심입니다.
...
이 세 줄을 주석으로라도 적어두면 테스트가 길어졌을 때도 흐름이 사라지지 않습니다. controller/service/repository 각각의 테스트가 모두 이 골격을 그대로 따릅니다 ([레이어별 테스트 전략](/posts/layered-testing-strategy)).
...
행위 검증은 강력합니다. 과용하면 테스트가 SUT 의 구현을 베껴 적은 또 하나의 사본이 됩니다. 같은 주제를 [의존성과 Testability](/posts/dependency-and-testability) 에서 다른 각도로 다시 짚습니다.
...
여기가 본론입니다. **Test Double** 은 실제 의존성을 대신해 테스트에서만 쓰이는 가짜 객체의 총칭입니다. Gerard Meszaros 의 분류가 가장 널리 쓰이며, 다섯 가지로 나뉩니다.
...
운영용 의존성과 **같은 인터페이스** 를 두고, 단순화된 동작을 들고 있는 객체입니다. 다음의 `FakeOrderRepository` 가 정확히 이 형태입니다.
...
새 테스트를 짤 때 `@MockBean` 또는 `mock()` 으로 협력자를 전부 가짜로 만들면, 테스트가 SUT 의 호출 시퀀스를 베껴 적은 사본이 됩니다. 도메인 협력자는 **Fake** 로, 진짜 외부 시스템(메일·결제·푸시) 만 **Stub/Mock** 으로 처리하는 결이 더 건강합니다. 이 분리를 처음부터 잡지 않았다면, 한 번 사고를 친 뒤 옮겨야 합니다. 저는 늦게 옮긴 쪽이었습니다.
...
`FakeOrderRepository` 가 점점 자라 트랜잭션·인덱스·페이징까지 흉내내기 시작하면, 어느 순간 운영 코드 못지않게 복잡해집니다. Fake 는 "운영 동작의 본질만 옮긴 가벼운 구현" 으로 유지하고, 진짜에 가까운 검증이 필요한 영역은 `@DataJpaTest` 같은 통합 테스트로 옮기는 편이 낫습니다. 이 경계는 [레이어별 테스트 전략](/posts/layered-testing-strategy) 에서 더 다룹니다.

## 회고 — 다섯 가지의 어휘가 PR 리뷰를 바꾼다

테스트 대역의 분류는 "이 협력자를 어떻게 다룰까" 라는 결정의 어휘입니다. 다섯 가지가 머리에 들어오기 전에는 PR 리뷰에서 "이건 mock 보다 fake 가 맞지 않을까" 같은 말이 나오지 않았습니다. 저는 그 어휘를 익히는 데 한 분기를 들였고, 그 뒤로 리뷰 코멘트의 평균 깊이가 한 단계 올라갔습니다.

SUT 가 협력자 6개를 `mock()` 으로 둘러싸야만 짜이는 상태라면, 대역의 문제가 아니라 SUT 의 의존성이 너무 많은 상태입니다. 같은 주제를 [의존성과 Testability](/posts/dependency-and-testability) 에서 더 구체적으로 다룹니다.

- **SUT** 는 테스트의 검증 대상, **BDD given-when-then** 은 그 검증의 골격.
- 검증은 **상태 vs 행위** 로 나뉘며 기본은 상태 검증, 부수효과가 본질인 경우에만 행위 검증.
- **Test Double** 다섯 가지 — Dummy/Fake/Stub/Mock/Spy — 는 자리에 맞춰 골라 씁니다.
- **Beyoncé Rule** — 지키고 싶다면, 테스트로 박아둡니다.
