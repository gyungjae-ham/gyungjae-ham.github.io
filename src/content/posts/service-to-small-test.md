...
> 2023-07 에 velog 에 정리한 글을 5-6년차 백엔드 시각으로 다시 손본 글입니다. 그때는 "이렇게 하면 빨라진다" 까지만 적었으나, 지금은 Mock 과 Fake 의 트레이드오프를 함께 적습니다.

**TL;DR.** `@SpringBootTest` + `h2` 로 20초씩 걸리던 Service 테스트를 Fake 기반 소형 테스트로 옮겨 0.05초까지 줄였습니다. 200배 빨라진 것보다 더 중요한 변화는, **Mock 으로는 검증 못 하던 "두 단계 시나리오"가 Fake 로는 자연스럽게 검증되더라는 점**이었습니다.

테스트 한 번 돌리면 IDE 가 30초간 멈춰 있었습니다. `@SpringBootTest` 가 매번 컨텍스트를 띄우고, `h2` 가 스키마를 만들고, `Liquibase` 가 마이그레이션을 돌리는 식이었습니다. 변경 한 줄에 30초씩 들이는 루프는 결국 **"테스트를 안 돌리고 PR 을 올리는 습관"** 으로 굳습니다. 이건 테스트가 느려서가 아니라, Service 가 자기 영역을 가지지 못한 채 인프라와 한 덩어리로 묶여 있었기 때문입니다.
...
테스트당 실행 시간은 **20초 → 0.05초** 로 떨어졌습니다. 변경 한 줄에 즉시 피드백이 옵니다. 빨라진 것보다 더 가치 있던 변화는, 메일 본문 안에 `fixed-uuid` 가 박혀 있는지까지 검증할 수 있게 된 점입니다. 기존 `verify(mailSender).send(any())` 는 "뭔가 보냈다" 까지만 알려줬을 뿐, 실제 본문이 의도대로 만들어졌는지는 검증한 적이 없었습니다.
...
소형 테스트로 옮긴 결정은 옳았습니다. 다만 한 가지 후회가 있습니다. **"Repository 인터페이스 분리" 를 처음부터 깔고 갔어야 했습니다.** 신규 프로젝트에서 이 분리를 미루면, 1년쯤 지나 옮길 때는 이미 도메인 코드 곳곳에 `JpaRepository` 의 메서드 시그니처가 박혀 있어 한 번에 옮기지 못합니다. 저는 그 비용을 두 번 지불한 적이 있습니다.

Fake 도 만능이 아닙니다. 진짜 RDB 의 unique 제약·트랜잭션·1차 캐시를 Fake 가 다 흉내내지는 못합니다. 그래서 Repository 자체의 통합 테스트 한 벌은 끝까지 남겨두는 것이 맞았습니다. 이 둘 — **소형 테스트의 속도 + 통합 테스트의 신뢰** — 을 함께 가져가는 균형이, 결국 가장 오래 유지되는 구조였습니다.

테스트 대역 다섯 가지 (Dummy / Fake / Stub / Mock / Spy) 의 구분은 [개념, 대역](/posts/test-doubles-concept) 글에서 정리했습니다.
