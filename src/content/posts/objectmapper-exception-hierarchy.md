...
> 이 글은 ObjectMapper 시리즈 3편 중 2편 입니다. 1편은 ObjectMapper 가 Spring MVC 의 어느 자리에 살고 있는지를 다뤘습니다 (slug: objectmapper-where-in-spring-mvc).

그때는 `JsonMappingException` 을 별도 catch 로 잡고 있었지만 그건 `JsonProcessingException` 의 자식이라 닿지도 않는 코드였습니다. 예외 계층을 정확히 보지 않은 채로 catch 를 늘리는 것이 가장 위험합니다. 다음 코드가 그때 그대로의 모양입니다.
...
부모 catch 가 먼저 걸리면 자식 catch 는 영원히 도달하지 않습니다. Java/Kotlin 의 catch 순서 규칙이고, 알면서도 이렇게 작성한 이유는 단순했습니다. 라이브러리 예외 계층을 직접 확인하지 않은 채로 "그래도 두 개 다 잡아 두면 안전하지 않을까" 라고 생각한 결이었습니다. 그 자리에서 한 번만 더 들여다봤어도 두 줄짜리 catch 가 사실은 한 줄이라는 사실이 보였을 것입니다.

## JsonProcessingException 의 정체부터

`JsonProcessingException` 은 Jackson 라이브러리가 JSON 을 처리할 때 발생하는 가장 기본적인 부모 예외 입니다. 어떤 상황에서 튀어 나오는지 정리하면 다음과 같습니다.

- JSON 문법이 잘못된 경우 (괄호 누락, 콤마 위치 오류 등)
- JSON 문자열이 중간에 끊긴 경우
- 입출력 작업 중 에러가 발생한 경우

이 예외가 발생하는 영역은 대부분 **클라이언트가 보낸 데이터의 문제** 입니다. 개발자가 코드 레벨에서 사전에 제어할 수 있는 자리가 아니라, 사용자 측 휴먼 에러나 네트워크 사건에 가깝습니다. 그래서 이 예외를 완전히 제거하는 일은 불가능하고, 결국 "무엇을 어디서 catch 하느냐" 의 문제로 좁혀집니다.

## JsonMappingException 은 자식이었습니다

`JsonMappingException` 은 Jackson 2.10 부터 `DatabindException` 을 상속하고, `DatabindException` 은 다시 `JsonProcessingException` 을 상속합니다. 계층은 다음과 같이 박혀 있습니다.

- `JsonProcessingException`
  - `DatabindException`
    - `JsonMappingException`

그러니까 위의 코드는 두 줄짜리 catch 가 아니라 사실상 한 줄짜리 catch 였습니다. 부모를 먼저 잡으면서 자식 catch 를 무력화하고 있었던 셈입니다. 자식만 따로 잡고 싶었다면 자식을 먼저 써야 했고, 다만 그렇게 분기를 늘리는 게 정말로 의미가 있는지부터 묻는 편이 낫습니다. 매핑 실패와 파싱 실패를 굳이 다른 에러 코드로 분기시킬 필요가 운영에서 실제로 있었는가를 되묻는 자리입니다. 대부분의 답은 "없다" 입니다.
...
설정으로 막을 수 없는 영역, 즉 진짜로 JSON 자체가 깨진 경우는 결국 try-catch 가 필요합니다. 다만 그 try-catch 가 **호출 지점마다 흩어지는 것** 이 문제이지, try-catch 자체가 문제는 아닙니다. 한 군데로 모으면 됩니다. `JsonConverter` 같은 Wrapper 컴포넌트가 그 자리입니다.
...
호출부는 책임을 명시하는 모양이 됩니다. 두 줄짜리 try-catch 가 흩어지지 않고, 변환의 책임이 한 컴포넌트 안에 모입니다.
...
그 자리에서 한 발 더 나가면, `@RestControllerAdvice` 와 결합해 `JsonProcessingException` 또는 `BusinessException` 을 일관된 에러 응답으로 변환할 수 있습니다. Wrapper 가 호출부 책임을 명시하게 하고, `ControllerAdvice` 가 최종 응답 형식을 책임지는 결입니다. 두 자리가 분리되어 있어야 catch 가 다시 흩어지지 않습니다.

## 정리

예외 catch 가 두 줄 이상 늘어났다면, 그건 예외 계층을 다시 봐야 한다는 신호 입니다. 두 예외가 부모-자식이라면 자식 catch 는 닿지 않고, 둘이 형제라면 분기 자체가 의미를 가지는지 다시 묻게 됩니다. 대부분의 경우 답은 "부모 하나로 충분하다" 이고, 그 부모 하나도 Wrapper 한 군데로 모으면 호출부에서 사라집니다. 결국 catch 줄을 늘리기 전에 예외의 부모를 먼저 보는 일이 가장 빠른 정리입니다.
