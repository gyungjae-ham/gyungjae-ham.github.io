...
> **TL;DR.** 테스트가 무거워지고 `@SpringBootTest` 가 안 빠지는 건 도구의 문제가 아니라 아키텍처의 문제입니다. 의존성 방향을 뒤집어 도메인이 인프라를 모르게 하면, `@SpringBootTest` 자리의 절반이 사라집니다. 헥사고날·클린의 핵심을 트레이드오프와 함께 정리합니다.

> 2023-06 에 velog 에 정리한 글을 5-6년차 백엔드 시각으로 다시 손본 글입니다. 원본의 짧은 메모를 그대로 옮기지 않고, 그때는 어렴풋했던 트레이드오프와 함정을 함께 적었습니다.

테스트가 점점 무거워지고 `@SpringBootTest` 가 빠질 자리가 없어지는 순간이 옵니다. 그때 `h2` 를 더 빠르게 만들거나 `Mockito` 를 더 정교하게 쓰는 길로 가기 쉽지만, 이건 **코드의 문제가 아니라 아키텍처가 보내는 신호**입니다.

## 테스트가 무거워질 때 봐야 할 것

운영 중인 프로젝트에서 다음 증상이 함께 나타난다면 원인은 분명합니다.
...
이건 도구의 한계가 아니라 **시스템이 RDB 와 외부 인프라에 강결합되어 있다는 증거**입니다. 정작 검증해야 할 도메인 로직이 인프라에 묶여 있어 분리된 검증이 불가능한 상태입니다.
...
이 코드를 단위 테스트하려면 6개 협력자를 모두 mocking 해야 하고, 통합 테스트하려면 모든 외부 시스템의 임베디드/스텁 환경이 필요합니다. **테스트가 무거워지는 직접적 원인**이 여기에 있습니다.

저는 이 fat service 를 1년 가까이 끼고 산 적이 있는데, 도메인 로직 한 줄 바꾸는 데 mock 셋업 30분이 걸리던 자리였습니다. 이게 옳지 않다는 건 알았지만 "지금 옮기면 일정이 안 맞는다" 를 반복하다 결국 통째로 뜯었습니다. 그 결정을 더 일찍 내렸어야 했습니다.
...
처음부터 헥사고날·클린을 풀세트로 깔지 않아도 됩니다. 다음 순서가 현실적이었습니다.
...
레이어드 아키텍처는 시작점으로 훌륭합니다. 다만 규모가 커지면 **DB 주도 설계 · 동시 작업 제약 · fat service** 라는 세 결함이 드러나고, 이걸 코드 문제로 풀려는 시도 (더 정교한 mock, 더 무거운 통합 테스트) 는 잠시 가려놓을 뿐 다시 옵니다. 진짜 해결은 **의존성 방향을 뒤집어 도메인이 인프라를 모르게 만드는 것**, 즉 헥사고날 · 클린 아키텍처의 한 줄입니다.

이 길이 모든 프로젝트의 정답은 아닙니다. 작은 CRUD 어드민에는 오버킬이고, 옮기는 동안 보일러플레이트와 러닝 커브 비용을 팀이 같이 짊어져야 합니다. 다만 외부 의존이 3개를 넘기고 테스트 실행이 1분을 넘기 시작했다면, 이미 옮길 시점은 지났습니다.
