...
> 2026-05 에 단독 설계·구현·머지까지 끝낸 athlog 영상 변환 파이프라인 회고입니다. 처음엔 AWS MediaConvert + EventBridge 비동기 알림이라는 그럴듯한 매니지드 청사진을 그렸지만, 정작 prod 에 들어간 건 `ffmpeg` + Celery indexing 큐였습니다. 그 결정 과정을 1인칭으로 풀어 둡니다.

athlog 의 인스타 스타일 영상이 모바일에서 첫 청크부터 100MB 를 통째로 다운로드받던 시기가 있었습니다. 데이터·체감 latency 모두 사용자에게 부담이라 노출 즉시 재생되는 구조가 필요했고, 그 길에서 **매니지드 서비스를 포기한** 회고를 남깁니다.
...
처음엔 "용량을 줄이면 풀리겠다" 고 생각해 FE 압축을 만지작거렸지만, **첫 프레임 5초는 용량의 문제가 아니라 적응형 스트리밍 부재가 보내는 신호** 였습니다. 영상이 짧아도 단일 mp4 는 전체 파일이 어느 정도 버퍼링되어야 재생이 시작되니, "잘게 자르고 점진적으로 다운로드" 라는 결을 선택해야 했습니다.
...
## 검토한 후보 — "규모가 크다" 와 "도구가 무거워야 한다" 는 다르다
...
Elastic Transcoder · GCP Transcoder · Cloudflare Stream · MUX 등 다른 매니지드도 후보에 올릴 수 있었지만, **영상 길이·트래픽 규모에 비해 명백히 과한 인프라라 일찍 기각** 했습니다. "큰 서비스 = 무거운 도구" 가 아니라 **"문제의 모양에 맞는 도구"** 가 결정 기준이었습니다.

## 처음 설계와 실제 구현의 거리 — 매니지드를 포기한 결정
...
- **dev 환경에서 MediaConvert 를 띄우는 부담** — 백엔드 1인이 dev·stg·prod 다 챙겨야 하는 상황에서 매니지드 외부 의존이 늘어남
- **테스트 트래픽 기준 월 $수십** 의 비용 — 작은 수치지만 "그만큼이라도 안 쓸 수 있으면 안 쓰자"

결정을 뒤집어 **B 후보 (`ffmpeg` + Celery indexing 큐)** 로 갔습니다. 설계 문서와 실제 구현이 다르다는 사실 자체가 약간 부끄러웠지만, **"매니지드가 답이라는 관성"** 에 의존하지 않고 한 번 더 의심한 결정이라 결과적으로 옳았다고 봅니다.

이 결정 뒤집기가 이 작업의 가장 어려운 자리였습니다. 단독 설계라는 건 외로운 만큼 자기 결정을 누가 검증해 주지 않는다는 의미이기도 합니다. "MediaConvert 가 답이라 적어둔 설계 문서를 내가 직접 뒤집어도 되는가" 를 한참 망설였습니다.

> 단독 설계의 책임은 결정을 내리는 것보다 **이미 내린 결정을 뒤집을 수 있는가** 에 있다는 걸 이 작업으로 배웠습니다.

## HLS 비트레이트 래더와 segment 길이 결정

세 단계 (1080p / 720p / 480p) 로 잡았습니다. **화질을 포기할 수 없었다** 는 게 1차 이유입니다. athlog 콘텐츠는 인스타 → 상품 PDP 로 이어지는 톱오브퍼널이라 화질이 콘텐츠의 신뢰도와 직결됩니다.
...
segment 길이는 **6초** 로 잡았습니다. Apple HLS Authoring Specification 의 권장값을 따랐습니다.
...
결과적으로 **HLS 개별 segment 가 2MB 미만** 으로 잘립니다. 100MB 한 덩어리를 50배 이상 경량화한 첫 청크입니다.
...
처음엔 변환 완료 시 자동 PUBLISHED 로 가려 했지만, **운영자가 의도적으로 HIDDEN 등록한 케이스가 무시되어 자동 공개되는 문제** 를 발견했습니다.

해결은 단순했습니다. 영상 포함 콘텐츠는 등록 시 무조건 HIDDEN 으로 INSERT 하고, 운영자가 입력한 status 는 `desired_status` 라는 태스크 인자로 캡처해 들고 갑니다. 변환 완료 시 그 `desired_status` 로 복원합니다.
...
이게 가장 만족스러운 결정이었습니다. 새 워커 클러스터를 띄우는 대신 **기존 OpenSearch 상품 indexing 큐가 대부분 idle 한 점을 활용** 했습니다.
...
> 단독 운영의 효율은 새 인프라를 깔지 않는 데서 나온다는 걸 이 자리에서 다시 확인했습니다.
...
영상 다운로드 단계에서 `requests` 로 외부 URL 을 받는 자리가 있습니다. SSRF 방어로 화이트리스트 + `allow_redirects=False` 를 적용했는데, **인스타 CDN 만 허용했더니 어드민 업로드의 자체 CDN 다운로드가 차단** 됐습니다.

운영 배포 후 첫 실패로 발각된 사고였습니다. 해결은 `PUBLIC_ASSET_URL` secret 에서 host suffix 를 동적 합성하도록 보강이었습니다.

이 사고에서 얻은 교훈은 **SSRF 화이트리스트는 처음부터 "신뢰하는 모든 출처" 를 빠짐없이 포함해야 한다** 는 점입니다. 인스타 CDN 만 보고 자체 CDN 을 누락한 것이 결정적 실수였습니다.
...
`validate_thumbnail_temp_key` 같은 공통 헬퍼로 추출해 두 흐름이 공유하도록 묶었습니다. 신규 규칙 `service/multi-entrypoint-parity.md` 로 사내 docs 에 승격해 후속 PR 에서 자가 점검 가능하게 했습니다.

## CloudFront · cache 정책 — 굳이 캐시를 안 한 이유

CloudFront 의 cache TTL 은 굳이 길게 잡지 않았습니다. **athlog 의 콘텐츠 뷰가 cursor pagination 기반** 이고, 사용자가 한 콘텐츠를 보면 앞뒤 콘텐츠가 미리 당겨진 상태로 로드되는 흐름이라 같은 영상에 대한 반복 히트가 적습니다.

따라서 cache 정책은 기본값을 유지하고, OAC (Origin Access Control) 로 S3 직접 접근을 막는 보안 기본만 적용했습니다.
...
- **측정 인프라를 처음부터 같이 깐다** — 첫 재생 시작 latency (LCP), 변환 task 평균·95p 소요 시간, 변환 실패율을 처음부터 대시보드로 묶었어야 했습니다. 설계는 잘 풀렸으나 정량 입증이 늦어졌습니다.
...
- **SSRF 화이트리스트는 동적 합성으로 처음부터** — 인스타 CDN 만 보고 시작했다가 첫 운영 사고로 자체 CDN 누락이 발각된 패턴, 다음에는 `_get_self_cdn_host_suffix` 같은 동적 합성을 처음부터 적용합니다.

## 회고 한 줄

매니지드가 답이라는 관성 대신 영상이 60초라는 사실을 다시 본 게 이 작업의 핵심이었습니다. 단독 설계의 진짜 자유는 새 인프라를 깔 자유가 아니라 **이미 그린 설계를 뒤집을 자유** 라는 걸, 이번에 확인했습니다.

같은 문제를 푸는 후배에게 한 줄을 준다면 — **"규모가 크다" 와 "도구가 무거워야 한다" 는 다르다.** 문제의 모양을 한 번 더 본 뒤 도구를 정하시기 바랍니다.

---

다음 글에서는 이 파이프라인의 prod 실측 LCP, 변환 task 95p 소요 시간, indexing 워커 CPU 추이를 정리합니다. 측정 인프라가 늦었다는 회고를 직접 메우는 글이 될 예정입니다.
